...
async def run_generate_platform_architecture_phase(args) -> None:
    ...
    for p in sorted(needs_generation, key=lambda x: x['name']):
        ...
        prompt = f"""The following context has been pre-gathered for this platform:

Distribution: {distribution}
Version: {version}
Generated By (use this exact string in Metadata): {model_display}
{build_context}
{relationship_context}The platform directory contains {p['component_count']} component architecture file(s).

The supported OCP versions dictate which OpenShift/Kubernetes APIs and platform
...
/aggregate-platform-architecture --platform-dir={platform_dir_path}"""
