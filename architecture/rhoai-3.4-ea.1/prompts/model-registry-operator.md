...
The operator applies the following overlay for RHOAI self-managed: overlays/odh

Overlay path within source_folder: overlays/odh
...
params.env values (from prefetched-manifests/modelregistry/overlays/odh/params.env):
  ...
  IMAGES_POSTGRES=quay.io/sclorg/postgresql-16-c10s:latest
  IMAGES_CATALOG_DATA=quay.io/opendatahub/odh-model-metadata-collection:latest
  IMAGES_BENCHMARK_DATA=quay.io/opendatahub/odh-model-metadata-collection:latest
  IMAGES_JOBS_ASYNC_UPLOAD=quay.io/opendatahub/model-registry-job-async-upload:latest
  CREATE_AUTH_RESOURCES=true
  DEFAULT_CERT=
  REGISTRIES_NAMESPACE=
  ENABLE_MODEL_CATALOG=
  SKIP_MODEL_CATALOG_DB_CREATION=
  GATEWAY_DOMAIN=

IMPORTANT: When analyzing this component's kustomize manifests, start from the overlay
kustomization.yaml at config/overlays/odh/ rather than the base. The overlay
