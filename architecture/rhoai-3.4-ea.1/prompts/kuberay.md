...
The operator applies the following overlay for RHOAI self-managed: openshift

Overlay path within source_folder: openshift
...
params.env values (from prefetched-manifests/ray/openshift/params.env):
...
kustomization.yaml at ray-operator/config/openshift/ rather than the base. The overlay
