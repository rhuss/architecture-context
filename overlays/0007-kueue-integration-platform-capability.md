...
- The RHOAI-specific enforcement is separate from Kueue's own admission control — in RHOAI 2.25 / 3.3, the platform enforces mandatory queue assignment policy via validating webhook; in 3.4 and next, platform admission-time enforcement is removed, while Kueue still requires queue assignment for scheduling
